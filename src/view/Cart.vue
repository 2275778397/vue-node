                    ...
                    <div class="cart-foot-inner">
                        <div class="cart-foot-l">
                            <div class="item-all-check">
                                ...
                                <a href="#" @click="delAllCart">批量删除</a>
                            </div>
                        </div>
                        <div class="cart-foot-r">
                            <div class="item-total">
                                共有: <span class="total-price">{{ checkedCount}}</span>件商品
                            </div>
                            <div class="item-total">
                                合计: <span class="total-price">{{totalPrice| currency('$')}}</span>
                            </div>
                            <div class="btn-wrap">
                                <a class="btn btn--m btn--red" v-bind:class="{'btn--dis':checkedCount==0}" @click="checkOut">下单</a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    data() {
        ...
        computed: {
            ...
            checkedCount() {
                ...
                this.$store.commit("initcartCount", k);
                return i;
            },
            totalPrice() {
                var money = 0;
        ...
        methods: {
            ...
            delCart() {
                axios.post("api/users/cartDel", {
                    ...
                    if (res.status == '0') {
                        this.modalConfirm = false;
                        this.$store.commit("updatecartCount", -parseInt(this.productNum));
                        this.init();
                    }
                });
            },
